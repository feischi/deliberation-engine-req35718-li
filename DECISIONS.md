...
**Config comparison is a first-class operation.** Config constants like `MIN_ROUNDS`, `MAX_ROUNDS`, and `MAX_ASSUMPTIONS_PER_ROUND` shape deliberation quality in ways that can't be evaluated by reading code alone. `compare.js` runs the same feature request through a matrix of config variants (defined in `compare-config.json`) and produces a side-by-side table of outcomes. This turns tuning from guesswork into a repeatable experiment. Variants run sequentially — parallel execution would interleave console output and make per-variant token cost unattributable.

**Token usage is tracked at the `callAgent` level.** All three agent roles flow through the single `callAgent` function, so capturing `response.usage` there ensures no call is missed. The total is attached to each decision document under `run_metadata.total_tokens`, making cost self-contained in the output. This matters both for production economics and for detecting degenerate runs — a high token count relative to rounds taken is a signal that the agents are cycling rather than converging.
